...
class Mpfour : IAnimListener{
    ...
    companion object {
        private const val TAG = "Mpfour"
        private lateinit var act:Activity
        private var fmap: HashMap<String, Mpfour> = hashMapOf()
        @JvmStatic public fun  play(activity: Activity, name: String, dir: String, mdfive: String)
        {
            if(fmap.containsKey(name))
            {
                var mf =  fmap.get(name)
                if(mf != null)
                {
                    mf.playImp(activity, name, dir, mdfive)
                }
            }
            else
            {
    ...
    private  fun  playImp(activity: Activity, name: String, dir: String, mdfive: String)
    {
        act = activity
        animView = AnimView(activity.applicationContext)
        loadFile(name, dir, mdfive)
    }
    // 动画View
    ...
    private fun loadFile(name: String, dir: String, mdfive: String) {
        val files = Array(1) {
            name
        }
        FileUtil.copyAssetsToStorage(act, dir, files) {
            uiHandler.post {
